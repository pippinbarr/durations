...
class MilleniumKeyboardist extends FlxSprite
{
	...
	public function new (X:Float,Y:Float,UserControlled:Bool = false):Void
	{
		...
		loadGraphic(GameAssets.MILLENIUM_KEYBOARD,true,false,9,9,true);
		replaceColor(0xFFFFFFFF,0xFFEECCFF);
		animation.add("idle",[0,0],10,true);
		animation.add("playing",[1,2,4,6,7,8,4,6,7,12,11,5,3,10,11,4,6,2,3],5,true);
		Helpers.setupSprite(this,8);
		animation.frameIndex = 0;
		animation.play("idle");

		aNote = new FlxSound();
		aNote.loadEmbedded(GameAssets.KEYBOARD_A,true);
		bNote = new FlxSound();
		bNote.loadEmbedded(GameAssets.KEYBOARD_B,true);
		dNote = new FlxSound();
		dNote.loadEmbedded(GameAssets.KEYBOARD_D,true);
		eNote = new FlxSound();
		eNote.loadEmbedded(GameAssets.KEYBOARD_E,true);
		fNote = new FlxSound();
		fNote.loadEmbedded(GameAssets.KEYBOARD_F,true);
	...
	private function handleMusic():Void
	{
		...
		if (currentFrames % BEAT_FRAME == 0)
		{
			...
			else
			{
				...
				noteProbability = Math.max(0.1,noteProbability - 0.1);
			}
