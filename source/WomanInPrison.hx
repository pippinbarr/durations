...
class WomanInPrison extends FlxText
{
	...
	private var currentPageStartIndex:Int = 0;
	private var previousPageStartIndex:Int = 0;


	public function new(X:Float,Y:Float,Width:Float,MaxHeight:Int)
	{			
	...
	private function showCurrentPage():Void
	{
		...
		var endOfPage:Int = Std.int(Math.min(novel.length,currentIndex+maxHeight));

		for (i in currentIndex...endOfPage)
		{
			text += novel[i] + "\n";
		}
	...
	{
		text = "";
		// currentIndex = Std.int(Math.max(currentIndex - maxHeight,0));
		currentIndex -= maxHeight;
		if (currentIndex < 0)
		{
			currentIndex = novel.length - 1 - (novel.length % maxHeight);
		}
		showCurrentPage();
	}
	...
	public static var novel:Array<String> = 
	[
"",
"",
"",
...
"opinion.",
"",
"A just, upright, and open management would secure the coöperation of",
